...
class Server {
    ...
    class GreeterImpl : RemoteDBGrpc.RemoteDBImplBase() {
        ...
        override fun addColumn(request: AddColumnRequest, responseObserver: StreamObserver<AddColumnResponse>) {
            ...
            val success = dbManager.addColumn(tableIndex, name, com.example.component.column.TypeColumn.valueOf(columnType.name))
        ...
        override fun changeColumnName(request: ChangeColumnNameRequest, responseObserver: StreamObserver<ChangeColumnNameResponse>) {
            val tableIndex = request.tableIndex
            val columnIndex = request.columnIndex
            val newName = request.newName
            val success = dbManager.changeColumnName(tableIndex, columnIndex, newName)
            val response = ChangeColumnNameResponse.newBuilder().setSuccess(success).build()
            responseObserver.onNext(response)
            responseObserver.onCompleted()
        }
