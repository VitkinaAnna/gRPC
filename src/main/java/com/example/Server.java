...
public class Server {
  ...
  static class GreeterImpl extends RemoteDBGrpc.RemoteDBImplBase {
      ...
      public void addColumn(AddColumnRequest request, StreamObserver<AddColumnResponse> responseObserver) {
          int tableIndex = request.getTableIndex();
          String name = request.getName();
          com.example.component.column.TypeColumn columnType = com.example.component.column.TypeColumn.valueOf(request.getColumnType().name()); // Make sure ColumnType enum matches your gRPC enum

          boolean success = dbManager.addColumn(tableIndex, name, columnType);
      ...
      @Override
      public void changeColumnName(ChangeColumnNameRequest request, StreamObserver<ChangeColumnNameResponse> responseObserver) {
          int tableIndex = request.getTableIndex();
          int columnIndex = request.getColumnIndex();
          String newName = request.getNewName();
          boolean success = dbManager.changeColumnName(tableIndex,columnIndex,newName);
          ChangeColumnNameResponse response = ChangeColumnNameResponse.newBuilder().setSuccess(success).build();
          responseObserver.onNext(response);
          responseObserver.onCompleted();
      }
